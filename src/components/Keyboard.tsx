...
export const Keyboard: React.FC<KeyboardProps> = ({ onChar, onDelete, onEnter, letterStatuses }) => {
  const getKeyClasses = (key: string) => {
    ...
    if (key === 'ENTER' || key === 'BACKSPACE') {
      return base + "px-3 sm:px-4 h-14 bg-zinc-200 hover:bg-zinc-300 text-xs sm:text-sm";
    }
    ...
    switch (status) {
      ...
      case 'absent': return base + "w-8 sm:w-10 h-14 bg-absent text-white";
      default: return base + "w-8 sm:w-10 h-14 bg-zinc-200 hover:bg-zinc-300 text-zinc-700";
    }
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-2 pb-8">
      ...
      <div className="flex justify-center mt-2">
        <button
          onClick={() => onChar('-')}
          className="w-full max-w-[200px] h-12 bg-zinc-200 hover:bg-zinc-300 rounded font-bold text-zinc-700 transition-colors flex items-center justify-center gap-2 uppercase text-xs tracking-widest"
        >
          Space / Dash
        </button>
      </div>
